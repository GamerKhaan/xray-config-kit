...
function isPublicListen(listen) {
...
function analyzeInboundSecurity(inbound, index) {
...
function analyzeRawExposure(profile) {
    const issues = [];
...
function analyzeRouting(profile) {
    ...
    if (domainRules.length > 0) {
        ...
        for (const [index, inbound] of profile.inbounds.entries()) {
            if (inbound.protocol === "unmanaged" || inbound.protocol === "http" || inbound.protocol === "mixed" || inbound.protocol === "socks" || inbound.protocol === "wireguard" || inbound.protocol === "tun" || inbound.protocol === "dokodemo-door" || inbound.protocol === "tunnel")
                continue;
            if (scopedTags.size > 0 && !scopedTags.has(inbound.tag))
                continue;
...
export function analyzeProfile(profile, options = {}) {
    ...
    if (validation.profile && audits.has("security")) {
        extraIssues.push(...validation.profile.inbounds.flatMap(analyzeInboundSecurity), ...analyzeRawExposure(validation.profile), ...analyzeRouting(validation.profile), ...analyzeDns(validation.profile));
    }
