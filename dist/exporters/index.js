...
function displayHost(inbound, host) {
    if (host)
        return host;
...
function singlePortForExport(inbound, override, label) {
...
function appendTransportParams(params, transport) {
    ...
    if (transport.type === "grpc") {
        ...
        if (transport.authority)
            params.set("authority", transport.authority);
        if (transport.multiMode)
...
function vmessTransportParams(transport) {
    if (transport.type === "tcp") {
...
function appendSecurityParams(params, security) {
    params.set("security", security.type);
    if (security.type === "tls") {
        if (security.serverName)
            params.set("sni", security.serverName);
        if (security.fingerprint)
            params.set("fp", security.fingerprint);
        if (security.alpn?.length)
            params.set("alpn", security.alpn.join(","));
        return;
    }
    if (security.type === "reality") {
        ...
        if (security.spiderX)
            params.set("spx", security.spiderX);
        if (security.fingerprint)
            params.set("fp", security.fingerprint);
        if (security.mldsa65Verify)
            params.set("pqv", security.mldsa65Verify);
    }
}
function linkRemark(inbound, client, options) {
    return options.remark ?? client.email ?? `${inbound.tag}-${client.protocol}`;
}
function shadowsocksUsesServerPassword(method) {
    return method === "2022-blake3-aes-128-gcm" || method === "2022-blake3-aes-256-gcm";
}
function shadowsocksClientPassword(inbound, client) {
    const method = client.method ?? inbound.method;
    if (!method)
        throw new Error("Shadowsocks client export requires a method on the client or inbound.");
    if (shadowsocksUsesServerPassword(method) && inbound.password)
        return `${inbound.password}:${client.password}`;
    return client.password;
}
export function generateClientLink(profile, options) {
    const inbound = findInbound(profile, options.inboundTag);
    if (!inbound)
        throw new Error(`Inbound "${options.inboundTag}" was not found or is unmanaged.`);
    const client = findClient(inbound, options.clientId);
    if (!client)
        throw new Error(`Client "${options.clientId}" was not found in inbound "${options.inboundTag}".`);
    if (client.enabled === false)
        throw new Error(`Client "${options.clientId}" is disabled.`);
    ...
    if (inbound.protocol === "shadowsocks" && client.protocol === "shadowsocks") {
        const method = client.method ?? inbound.method;
        if (!method)
            throw new Error("Shadowsocks link generation requires a method on the client or inbound.");
...
export function generateVmessLink(profile, options) {
    const inbound = findInbound(profile, options.inboundTag);
    if (inbound?.protocol !== "vmess")
        throw new Error(`Inbound "${options.inboundTag}" is not a VMess inbound.`);
    return generateClientLink(profile, options);
}
export function generateVlessLink(profile, options) {
    const inbound = findInbound(profile, options.inboundTag);
    if (inbound?.protocol !== "vless")
        throw new Error(`Inbound "${options.inboundTag}" is not a VLESS inbound.`);
    return generateClientLink(profile, options);
}
export function generateTrojanLink(profile, options) {
    const inbound = findInbound(profile, options.inboundTag);
    if (inbound?.protocol !== "trojan")
        throw new Error(`Inbound "${options.inboundTag}" is not a Trojan inbound.`);
    return generateClientLink(profile, options);
}
export function generateShadowsocksLink(profile, options) {
    const inbound = findInbound(profile, options.inboundTag);
    if (inbound?.protocol !== "shadowsocks")
        throw new Error(`Inbound "${options.inboundTag}" is not a Shadowsocks inbound.`);
    return generateClientLink(profile, options);
}
export function generateWireGuardConfig(profile, options) {
    const inbound = findInbound(profile, options.inboundTag);
    if (inbound?.protocol !== "wireguard")
        throw new Error(`Inbound "${options.inboundTag}" is not a WireGuard inbound.`);
    const peer = inbound.peers.find((candidate) => candidate.publicKey === options.peerPublicKey);
    if (!peer)
        throw new Error(`WireGuard peer "${options.peerPublicKey}" was not found in inbound "${options.inboundTag}".`);
    const serverPublicKey = options.serverPublicKey ?? inbound.publicKey;
    if (!serverPublicKey)
        throw new Error("WireGuard config generation requires inbound.publicKey or options.serverPublicKey.");
...
function compileRealityClient(security) {
    if (!security.publicKey)
        throw new Error("REALITY xray-json subscription requires security.publicKey.");
    return {
...
function compileClientStream(transport, security) {
    if (security.type !== "reality")
        return compileStreamSettings(transport, security);
...
function outboundTag(inbound, client) {
...
function compileClientOutbound(inbound, client, host) {
    ...
    if (inbound.protocol === "shadowsocks" && client.protocol === "shadowsocks") {
        const method = client.method ?? inbound.method;
        if (!method)
            throw new Error("Shadowsocks outbound subscription generation requires a method on the client or inbound.");
        const outbound = {
            tag: outboundTag(inbound, client),
...
function selectedClients(profile, options) {
    ...
    for (const inbound of profile.inbounds) {
        ...
        for (const client of clients) {
            ...
            if (client.enabled === false && !options.includeDisabled)
                continue;
            if (selected.size > 0 && !ids.some((id) => selected.has(id)))
                continue;
...
export function generateSubscription(profile, options) {
    const issues = [];
    ...
    if (options.format === "xray-json") {
        ...
        const config = buildXrayConfig({
            ...
            outbounds: [
                ...
                ...outbounds.map((raw, index) => ({ protocol: "unmanaged", tag: `client-${index}`, editable: false, raw }))
            ]
