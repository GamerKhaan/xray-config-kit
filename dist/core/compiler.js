...
function shouldSkipMissingSourceField(parentKey, key, value, source) {
...
const knownSourceFieldsByParent = new Map([
...
function shouldDropMissingCompiledField(parentKey, key) {
    return knownSourceFieldsByParent.get(parentKey ?? "__root")?.has(key) === true;
}
function mergeJsonPreservingSource(source, compiled, parentKey) {
    ...
    if (isJsonObject(source) && isJsonObject(compiled)) {
        ...
        for (const key of Object.keys(source)) {
            if (compiled[key] === undefined && shouldDropMissingCompiledField(parentKey, key))
                delete output[key];
        }
        for (const [key, value] of Object.entries(compiled)) {
            ...
            if (sourceValue === undefined) {
                if (shouldSkipMissingSourceField(parentKey, key, value, source))
                    continue;
...
function compileFallback(fallback) {
...
function compileSniffing(sniffing) {
    if (!sniffing)
        return undefined;
    return compactObject({
...
function compileRealityServer(security) {
    return compactObject({
        show: security.show,
...
function compileTcp(transport) {
    return compactObject({
        acceptProxyProtocol: transport.acceptProxyProtocol,
        header: transport.header
            ? compactObject({
                type: transport.header.type,
                request: transport.header.request ? compactObject(transport.header.request) : undefined,
                response: transport.header.response ? compactObject(transport.header.response) : undefined
            })
            : undefined
    });
}
function compileGrpc(transport) {
    return compactObject({
...
function compileXhttp(transport) {
    return compactObject({
...
function compileWebSocket(transport) {
    return compactObject({
        path: transport.path,
        host: transport.host,
...
function compileHttpUpgrade(transport) {
    return compactObject({
        path: transport.path,
        host: transport.host,
        headers: transport.headers,
...
function compileQuicParams(params) {
    return compactObject({
...
function compileHysteria(transport) {
    return compactObject({
...
export function compileStreamSettings(transport, security, advanced) {
    const streamSettings = {
        network: transport.type,
        security: security.type
    };
    if (security.type === "tls")
        streamSettings.tlsSettings = compileTls(security);
    if (security.type === "reality")
        streamSettings.realitySettings = compileRealityServer(security);
    if (transport.type === "tcp")
        streamSettings.tcpSettings = compileTcp(transport);
    if (transport.type === "grpc")
        streamSettings.grpcSettings = compileGrpc(transport);
    if (transport.type === "xhttp")
        streamSettings.xhttpSettings = compileXhttp(transport);
    if (transport.type === "ws")
        streamSettings.wsSettings = compileWebSocket(transport);
    if (transport.type === "httpupgrade")
        streamSettings.httpupgradeSettings = compileHttpUpgrade(transport);
    if (transport.type === "kcp")
        streamSettings.kcpSettings = compileKcp(transport);
    if (transport.type === "hysteria")
        streamSettings.hysteriaSettings = compileHysteria(transport);
    if (advanced?.sockopt)
        streamSettings.sockopt = advanced.sockopt;
...
function compileInboundBase(inbound, settings, security, transport) {
...
function compileInbound(inbound) {
    if (inbound.protocol === "unmanaged")
        return cloneJson(inbound.raw);
    ...
    if (inbound.protocol === "vless") {
        const inboundFlow = typeof inbound.flow === "string" && inbound.flow.trim() !== "" ? inbound.flow.trim() : undefined;
        const settings = compactObject({
            clients: inbound.clients
                .filter((client) => client.enabled !== false)
                .map((client) => compactObject({
    ...
    if (inbound.protocol === "dokodemo-door" || inbound.protocol === "tunnel") {
        const portMap = inbound.portMap !== undefined && Object.keys(inbound.portMap).length > 0 ? inbound.portMap : undefined;
    ...
    const shadowsocksInbound = inbound;
    const settings = compactObject({
...
function compileOutbound(outbound) {
    if (outbound.protocol === "unmanaged")
        return cloneJson(outbound.raw);
    let compiled = compactObject({
        tag: outbound.tag,
        protocol: outbound.protocol,
        sendThrough: outbound.sendThrough,
        settings: outbound.settings && Object.keys(outbound.settings).length > 0 ? cloneJson(outbound.settings) : undefined,
...
function compileRouting(profile) {
    if (!profile.routing)
        return undefined;
    return compactObject({
...
function applyUnknownPreservation(config, profile) {
    let next = config;
    const issues = [];
    for (const [path, value] of Object.entries(profile.unknown?.pointers ?? {})) {
        try {
            next = applyRawPatch(next, { op: "add", path: path, value });
        }
        catch (error) {
...
export function buildXrayConfig(profileInput, options = {}) {
    ...
    const sourcePreserved = normalized.raw?.source
        ? mergeJsonPreservingSource(normalized.raw.source, compiledProfile)
        : compiledProfile;
    let config = mergeTopLevel(sourcePreserved, normalized.raw?.topLevel);
    const issues = [...analysis.issues];
    if (normalized.raw?.source &&
        ...
        config = cloneJson(normalized.raw.source);
    }
    ...
    for (const patch of normalized.raw?.patches ?? []) {
        if (patch.unsafe && !options.allowUnsafeRaw)
            continue;
        try {
