...
function clientKey(client) {
    if (client.protocol === "vmess")
        return `vmess:${client.id.toLowerCase()}`;
    if (client.protocol === "vless")
        return `vless:${client.id.toLowerCase()}`;
    if (client.protocol === "trojan")
        return `trojan:${client.password}`;
...
function validateRawPatches(patches, path, options) {
    if (!patches)
        return [];
...
function validateInboundTags(profile) {
    const seen = new Map();
    const issues = [];
...
function validateOutboundTags(profile) {
    const seen = new Map();
    const issues = [];
...
function validateClients(inbound, inboundIndex) {
...
function validateReality(inbound, inboundIndex) {
...
function validateFallbacks(inbound, inboundIndex) {
    if (inbound.protocol === "unmanaged" || !("fallbacks" in inbound))
        return [];
    return (inbound.fallbacks ?? []).flatMap((fallback, fallbackIndex) => {
        if (!fallback.path || fallback.path.startsWith("/"))
            return [];
        return [
            makeIssue({
...
function isValidPortSegment(segment) {
    const trimmed = segment.trim();
    if (trimmed === "")
        return false;
    if (envPortRegex.test(trimmed))
        return true;
    if (/^\d+$/.test(trimmed))
        return isValidPortNumber(Number(trimmed));
...
function validatePortList(port, path, label) {
    if (typeof port === "number" && isValidPortNumber(port))
        return [];
    if (typeof port === "string" && port.split(",").every(isValidPortSegment))
        return [];
    return [
        makeIssue({
...
function validateInboundPort(port, inboundIndex) {
    return validatePortList(port, `/inbounds/${inboundIndex + 1}/port`, "Inbound port");
}
function validateRoutingRule(rule, ruleIndex) {
    const issues = [];
...
function validateRoutingBalancers(profile) {
...
function validateLocalInbound(inbound, inboundIndex) {
    if (inbound.protocol === "unmanaged")
        return [];
    const issues = [];
...
function semanticIssues(profile, options) {
    return [
...
export function validateProfile(input, options = {}) {
    ...
    if (!parsed.success) {
        return {
            ...
            issues: parsed.error.issues.map((zodIssue) => makeIssue({
                ...
                severity: "error",
                category: "schema",
                path: pathForZod(zodIssue.path.filter((part) => typeof part === "string" || typeof part === "number")),
